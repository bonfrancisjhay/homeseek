...
/** Returns true if the given date string (YYYY-MM-DD) is in the bookedDates array */
function isDateBlocked(dateStr, bookedDates) {
  return bookedDates.includes(dateStr);
}

/** Returns the earliest available date on or after `from` that is not blocked */
function nextAvailableDate(from, bookedDates) {
  const d = new Date(from);
  for (let i = 0; i < 365; i++) {
    const str = d.toISOString().split('T')[0];
    if (!isDateBlocked(str, bookedDates)) return str;
...
export default function ListingDetail() {
  ...
  const handleChange = e => {
    ...
    if (name === 'check_in') {
      // If new check_in is after current check_out, reset check_out
      setForm(prev => ({
        ...prev,
        check_in:  value,
  ...
  const rangeHasBlockedDates = (checkIn, checkOut) => {
    if (!checkIn || !checkOut) return false;
    const start = new Date(checkIn);
    const end   = new Date(checkOut);
    const d     = new Date(start);
    while (d <= end) {
      if (isDateBlocked(d.toISOString().split('T')[0], bookedDates)) return true;
      d.setDate(d.getDate() + 1);
    }
    return false;
  };
  ...
  const handleBooking = async e => {
    ...
    // Client-side date overlap guard
    if (rangeHasBlockedDates(form.check_in, form.check_out)) {
      setError('Your selected dates include already-booked nights. Please choose different dates.');
      return;
    }
            ...
            <div className="border border-blue-100 rounded-2xl overflow-hidden shadow-[0_8px_40px_rgba(59,130,246,0.15)] sticky top-24">
              ...
              {success ? (
                <div className="text-center py-10 px-6">
                  ...
                  <p className="text-gray-400 text-sm mb-2">Check your email for confirmation details.</p>
                  <p className="text-gray-500 text-sm mb-6">
                    {form.check_in} → {form.check_out} · {form.guests} guest{form.guests > 1 ? 's' : ''}
                  </p>
                  <button
                    onClick={() => navigate('/my-bookings')}
                    className="w-full py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-semibold text-sm transition mb-3"
                  >
                    View my bookings
                  </button>
                  <button
                    ...
                    <form onSubmit={handleBooking}>
                      ...
                      {bookedDates.length > 0 && (
                        <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 mb-4">
                          ⚠️ Some dates are already booked. Unavailable dates will be rejected.
                        </p>
                      )}
                      ...
                      {form.check_in && form.check_out && rangeHasBlockedDates(form.check_in, form.check_out) && (
                        <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-xl px-3 py-2 mb-4">
                          ❌ Your selected dates include nights that are already booked.
                        </p>
                      )}
                      ...
                      <button
                        type="submit"
                        disabled={booking || (form.check_in && form.check_out && rangeHasBlockedDates(form.check_in, form.check_out))}
